...
class multiRobot:
    ...
    def multiRobotPlanningNinjaTechnique(self):
            ...
            for j in range(i+1, self.nosRobos):
                ...
                while collide == 1:
                    for k in range(numberPoints-1):
                        ...
                        if(dstar[i].path[k] == dstar[j].path[k] and dstar[i].path[k] != dstar[i].path[-1]):
                            print("Collision" , i , j)
                            print(dstar[i].path[k], dstar[j].path[k])
                            self.rePlan(self.starts[i], self.goals[i],dstar[i], dstar[i].path[k])
                            dstar[i].path += [dstar[i].path[-1]] * \
                                (numberPoints - len(dstar[i].path))
                            # print(dstar[i].path)
                            collide = 1
                        ...
                        if(dstar[i].path[k] == dstar[j].path[k+1] and dstar[i].path[k+1] == dstar[j].path[k] and dstar[i].path[k] != dstar[i].path[-1]):
                            print("Collision Interchange", i, j)
                            print(dstar[i].path[k], dstar[i].path[k+1])
                            self.rePlan(
                                self.starts[i], self.goals[i], dstar[i], dstar[i].path[k])
                            dstar[j].path += [dstar[i].path[-1]] * \
...
def plan():
    ...
    #                         [(3, 5), (4, 10)]]
    #                         )
    return instance1.multiRobotPlanningNinjaTechnique()
    # s_start = (1, 5)
    # s_goal = (10, 10)
