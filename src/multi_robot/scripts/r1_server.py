...
def move(px,py):
    ...
    #robot_1 = (2,10) # Initial Point of robot 1
    ...
    while (abs(yaw1-math.atan2((point[1]-r1y),(point[0]-r1x)))> 0.01) and not rospy.is_shutdown():
        vel_msg.angular.z = limitTurn(kpz*(clipPi(math.atan2((point[1]-r1y),(point[0]-r1x))- yaw1)))
...
if __name__ == '__main__':
    rospy.init_node('goto1')
    server = gotoAction(rospy.get_name())
    first_velocity_publisher = rospy.Publisher('/first_robot/cmd_vel', Twist, queue_size=10)
    first_velocity_sub = rospy.Subscriber ('/first_robot/odom', Odometry, get_rotation_first_robot)
    rospy.spin()